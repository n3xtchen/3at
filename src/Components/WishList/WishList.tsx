...
export default function WishList({changeHref}) {
    ...
    let {wishItems, setWishItems,

      getLoggedUserWishList,addToWishList,deleteFromWishList ,  setNumOfWishListItems} =  useContext(wishlistContext);
     ...
     wishItems.length>0?
      <div className='mt-5'>
        ...
        <div className='row justify-content-center mt-5'>
          { wishItems.map((product, i)=><div key={i} className='col-md-2 h-25 '>
            <div className='product py-3 px-2 position-relative px-3'>
              ...
              {addedProduct == product.id ? <button className='btn bg-main text-white w-100 my-2'><i className='fas fa-spinner fa-spin'></i> Adding... </button>:<button onClick={()=>addProduct(product._id)} className='btn bg-main text-white w-100 my-2'>Add to Cart + </button>}
            </div>
          </div>)}
        </div>
